...
const registerUser = async(req,res)=>{
    ...
    if (user) return res.status(400).json("User with the given Email already in use...");

    if (!name || !email || !password) return res.status(400).json("All field are required...");

    if (!validator.isEmail(email)) return res.status(400).json("Email must be a valide email...");
...
const loginUser = async(req,res)=>{
    ...
    try {
        ...
        if(!isValidePassword) return res.status(400).json('Invalid email or password...');
